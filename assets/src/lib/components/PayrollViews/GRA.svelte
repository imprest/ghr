...
<div class="section sm:block md:flex lg:gap-4">
	...
	<p>
		Total Income: {moneyFmt(summary.taxable_income + msummary.taxable_income)} | Tax: {moneyFmt(
			summary.total_tax + msummary.total_tax
		)}
	</p>
</div>
		...
		{#each payroll.data as p, i}
			<tr class="group/item">
				...
				<td class="text-center"
					>{#if Number.parseFloat(p.ssnit_amount) > 0}Y{:else}N{/if}</td
				>
