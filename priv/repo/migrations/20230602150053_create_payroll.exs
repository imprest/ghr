defmodule Ghr.Repo.Migrations.CreatePayroll do
  ...
  def change do
    create table(:emps, primary_key: false) do
      ...
      add :dept, :text, null: false, default: "Administration"
      # MR, DRIVER, MECHANIC, CEO, MANAGER etc
      add :sub_dept, :text, null: false, default: "Worker"
      add :designation, :text, null: true
    ...
    create index(:months, [:org_id])
    create unique_index(:months, [:org_id, :month])

    create table(:months_payroll, primary_key: false) do
      # org_id_YYYYMM_emp_id
      add :id, :text, primary_key: true
      add :month_id, references(:months, on_delete: :nothing, type: :string), null: false
      ...
      # ADMIN, FACTORY, SALES, BSV
      add :group, :text, null: false, default: "ADMIN"
      # MR, DRIVER, MECHANIC, CEO, MANAGER etc
      add :sub_group, :text, null: false, default: "WORKER"
      add :remarks, :text
      add :inserted_by_id, references(:users, on_delete: :nothing)
      add :updated_by_id, references(:users, on_delete: :nothing)
      timestamps()
    ...
    create index(:months_payroll, [:month_id])
    create unique_index(:months_payroll, [:month_id, :emp_id])

    create table(:overtimes, primary_key: false) do
      # org_id_YYYYMM_emp_id
      add :id, :text, primary_key: true
      add :month_id, references(:months, on_delete: :nothing, type: :string), null: false
      add :emp_id, references(:emps, on_delete: :nothing, type: :string), null: false
      ...
      # 1.5 for normal | 2 for holidays
      add :rate, :decimal, null: false, default: 1.50, precision: 20, scale: 2
      add :inserted_by_id, references(:users, on_delete: :nothing)
      add :updated_by_id, references(:users, on_delete: :nothing)
      timestamps()
    end

    create index(:overtimes, [:month_id])
    create index(:overtimes, [:emp_id])
    create unique_index(:overtimes, [:month_id, :emp_id])
  end
end
