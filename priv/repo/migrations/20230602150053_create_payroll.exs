defmodule Ghr.Repo.Migrations.CreatePayroll do
  ...
  create table(tax_rates) do
    ...
    # effective date of income tax rates 
    ...
    add :overtime, :binary, null: false
    # Also embedded bonus tax rate 
    add :bonus, :binary, null: false
    add :inserted_by_id, references(:users, on_delete: :nothing)
    add :updated_by_id, references(:users, on_delete: :nothing)
    timestamps()
  end
  create index(:tax_rates, [:org_id])
  ...
  create table(:emp_master) do
    ...
    add :bonus_tax_rate, :decimal, null: false, default: 5.00, precision: 20, scale: 2 # just set to 5% according to law
